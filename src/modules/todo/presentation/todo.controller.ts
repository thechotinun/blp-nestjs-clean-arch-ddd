...
export class TodoController {
	...
	findAll(@Query() query: PaginationQueryDto): Promise<Paginated<TodoView>> {
		return this.listTodos.execute(query.toParams(this.configService.get<number>('PER_PAGE')!));
	}
	...
	@Patch(':id')
	update(@Param('id', ParseUUIDPipe) id: string, @Body() dto: UpdateTodoDto): Promise<TodoView> {
