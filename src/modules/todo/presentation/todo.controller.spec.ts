...
describe('TodoController (HTTP)', () => {
	...
	it('POST should reject invalid body and unknown fields', async () => {
		const res = await request(app.getHttpServer()).post(url).send({ title: '', foo: 1 });
		...
		expect(res.body.error.errors).toEqual(
			expect.arrayContaining(['property foo should not exist', 'title should not be empty']),
		);
	});
	...
	it('POST should map the domain title rule to its catalog code', async () => {
		const res = await request(app.getHttpServer()).post(url).send({ title: '   ' });
