...
describe('API response envelope', () => {
	...
	it('should return empty last link when there are no items', async () => {
		const res = await request(app.getHttpServer()).get('/api/v1/examples/empty');
	...
	it('should map a domain exception to its status and business code', async () => {
		const res = await request(app.getHttpServer()).get('/api/v1/examples/missing');
	...
	it('should put validation messages into errors', async () => {
		const res = await request(app.getHttpServer()).get('/api/v1/examples/invalid');
